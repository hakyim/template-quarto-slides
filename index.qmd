...
## Leeuw et al report inflation in TWAS  {background-image=assets/slide-background.svg}
...
- $Y = \sum_k X_k  ~\delta_k +  \epsilon$  
- $T = X_1$   

<br>

- when we regress $Y$ on $T$, $T$ will capture the association between $X_1$ and $Y$ 

- for small sample sizes, the effect could be negligible but with biobank-scale data, the inflation will be more prominent
...
## We can do the math and compute under the null {background-image=assets/slide-background.svg}
...
## Let's compare central to noncentral distributions {background-image=assets/slide-background.svg}

![](assets/central-vs-noncentral-chi2.png)

## Simulated Null χ2 vs Prediction {background-image=assets/slide-background.svg}

![](assets/simulated-null-chi2-vs-prediction.png)

## Null TWAS χ2 vs Prediction {background-image=assets/slide-background.svg}
